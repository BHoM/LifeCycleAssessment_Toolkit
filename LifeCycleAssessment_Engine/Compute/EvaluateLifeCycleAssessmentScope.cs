...
namespace BH.Engine.LifeCycleAssessment
{
    public static partial class Compute
    {
        ...
        public static List<LifeCycleAssessmentElementResult> EvaluateLifeCycleAssessmentScope(this StructuresScope structuresScope, EnvironmentalProductDeclarationField field, List<LifeCycleAssessmentPhases> phases, bool exactMatch = false)
        {
            ...
            List<IElementM> beams = structuresScope.Beams.Select(x => x).ToList();
            ...
            for (int x = 0; x < beamResults.Count; x++)
            {
                ...
                EnvironmentalMetricResult beamQuantity = (EnvironmentalMetricResult)EvaluateEnvironmentalProductDeclaration(beams[x],phases,field,exactMatch);

                beamResults[x] = new EnvironmentalMetricResult(beamResults[x].ObjectId, beamResults[x].ResultCase, beamResults[x].TimeStep, ObjectScope.Structure, ObjectCategory.Beam, phases, beamResults[x].EnvironmentalProductDeclaration, beamQuantity.Quantity, field, beams);
            }
            ...
            List<IElementM> columns = structuresScope.Columns.Select(x => x).ToList();
            ...
            for (int x = 0; x < columnsResults.Count; x++)
            {
                ...
                columnsResults[x] = new EnvironmentalMetricResult(columnsResults[x].ObjectId, columnsResults[x].ResultCase, columnsResults[x].TimeStep, ObjectScope.Structure, ObjectCategory.Column, phases, columnsResults[x].EnvironmentalProductDeclaration, (columnsResults[x] as EnvironmentalMetricResult).Quantity, field, columns);
            }
            ...
            List<IElementM> slabs = structuresScope.Slabs.Select(x => x).ToList();
            ...
            for (int x = 0; x < slabsResults.Count; x++)
            {
                ...
                slabsResults[x] = new EnvironmentalMetricResult(slabsResults[x].ObjectId, slabsResults[x].ResultCase, slabsResults[x].TimeStep, ObjectScope.Structure, ObjectCategory.Slab, phases, slabsResults[x].EnvironmentalProductDeclaration, (slabsResults[x] as EnvironmentalMetricResult).Quantity, field, slabs);
            }
            ...
            List<IElementM> coreWalls = structuresScope.CoreWalls.Select(x => x).ToList();
            ...
            for (int x = 0; x < coreWallsResults.Count; x++)
            {
                ...
                coreWallsResults[x] = new EnvironmentalMetricResult(coreWallsResults[x].ObjectId, coreWallsResults[x].ResultCase, coreWallsResults[x].TimeStep, ObjectScope.Structure, ObjectCategory.Wall, phases, coreWallsResults[x].EnvironmentalProductDeclaration, (coreWallsResults[x] as EnvironmentalMetricResult).Quantity, field, coreWalls);
            }
            ...
            List<IElementM> bracing = structuresScope.Bracing.Select(x => x).ToList();
            ...
            for (int x = 0; x < bracingResults.Count; x++)
            {
                ...
                bracingResults[x] = new EnvironmentalMetricResult(bracingResults[x].ObjectId, bracingResults[x].ResultCase, bracingResults[x].TimeStep, ObjectScope.Structure, ObjectCategory.Bracing, phases, bracingResults[x].EnvironmentalProductDeclaration, (bracingResults[x] as EnvironmentalMetricResult).Quantity, field, bracing);
            }
            ...
            List<IElementM> objects = structuresScope.AdditionalObjects.Select(x => x).ToList();
            ...
            for (int x = 0; x < additionalObjectsResults.Count; x++)
            {
                ...
                additionalObjectsResults[x] = new EnvironmentalMetricResult(additionalObjectsResults[x].ObjectId, additionalObjectsResults[x].ResultCase, additionalObjectsResults[x].TimeStep, ObjectScope.Structure, ObjectCategory.AdditionalObjects, phases, additionalObjectsResults[x].EnvironmentalProductDeclaration, (additionalObjectsResults[x] as EnvironmentalMetricResult).Quantity, field, objects);
            }
        ...
        public static List<LifeCycleAssessmentElementResult> EvaluateLifeCycleAssessmentScope(this FoundationsScope foundationsScope, EnvironmentalProductDeclarationField field, List<LifeCycleAssessmentPhases> phases, bool exactMatch = false)
        {
            ...
            List<IElementM> columns = foundationsScope.Columns.Select(x => x).ToList();
            ...
            for (int x = 0; x < columnResults.Count; x++)
            {
                ...
                columnResults[x] = new EnvironmentalMetricResult(columnResults[x].ObjectId, columnResults[x].ResultCase, columnResults[x].TimeStep, ObjectScope.Foundation, ObjectCategory.Column, phases, columnResults[x].EnvironmentalProductDeclaration, (columnResults[x] as EnvironmentalMetricResult).Quantity, field, columns);
            }
            ...
            List<IElementM> footings = foundationsScope.Footings.Select(x => x).ToList();
            ...
            for (int x = 0; x < footingsResults.Count; x++)
            {
                ...
                footingsResults[x] = new EnvironmentalMetricResult(footingsResults[x].ObjectId, footingsResults[x].ResultCase, footingsResults[x].TimeStep, ObjectScope.Foundation, ObjectCategory.Footing, phases, footingsResults[x].EnvironmentalProductDeclaration, (footingsResults[x] as EnvironmentalMetricResult).Quantity, field, footings);
            }
            ...
            List<IElementM> piles = foundationsScope.Piles.Select(x => x).ToList();
            ...
            for (int x = 0; x < pilesResults.Count; x++)
            {
                ...
                pilesResults[x] = new EnvironmentalMetricResult(pilesResults[x].ObjectId, pilesResults[x].ResultCase, pilesResults[x].TimeStep, ObjectScope.Foundation, ObjectCategory.Pile, phases, pilesResults[x].EnvironmentalProductDeclaration, (pilesResults[x] as EnvironmentalMetricResult).Quantity, field, piles);
            }
            ...
            List<IElementM> walls = foundationsScope.Walls.Select(x => x).ToList();
            ...
            for (int x = 0; x < wallsResults.Count; x++)
            {
                ...
                wallsResults[x] = new EnvironmentalMetricResult(wallsResults[x].ObjectId, wallsResults[x].ResultCase, wallsResults[x].TimeStep, ObjectScope.Foundation, ObjectCategory.Wall, phases, wallsResults[x].EnvironmentalProductDeclaration, (wallsResults[x] as EnvironmentalMetricResult).Quantity, field, walls);
            }
            ...
            List<IElementM> slabs = foundationsScope.Slabs.Select(x => x).ToList();
            ...
            for (int x = 0; x < fndSlabsResults.Count; x++)
            {
                ...
                fndSlabsResults[x] = new EnvironmentalMetricResult(fndSlabsResults[x].ObjectId, fndSlabsResults[x].ResultCase, fndSlabsResults[x].TimeStep, ObjectScope.Foundation, ObjectCategory.Slab, phases, fndSlabsResults[x].EnvironmentalProductDeclaration, (fndSlabsResults[x] as EnvironmentalMetricResult).Quantity, field, slabs);
            }
            ...
            List<IElementM> beams = foundationsScope.GradeBeams.Select(x => x).ToList();
            ...
            for (int x = 0; x < gradeBeamsResults.Count; x++)
            {
                ...
                gradeBeamsResults[x] = new EnvironmentalMetricResult(gradeBeamsResults[x].ObjectId, gradeBeamsResults[x].ResultCase, gradeBeamsResults[x].TimeStep, ObjectScope.Foundation, ObjectCategory.GradeBeam, phases, gradeBeamsResults[x].EnvironmentalProductDeclaration, (gradeBeamsResults[x] as EnvironmentalMetricResult).Quantity, field, beams);
            }
            ...
            List<IElementM> objects = foundationsScope.AdditionalObjects.Select(x => x).ToList();
            ...
            for (int x = 0; x < additionalObjectsResults.Count; x++)
            {
                ...
                additionalObjectsResults[x] = new EnvironmentalMetricResult(additionalObjectsResults[x].ObjectId, additionalObjectsResults[x].ResultCase, additionalObjectsResults[x].TimeStep, ObjectScope.Foundation, ObjectCategory.AdditionalObjects, phases, additionalObjectsResults[x].EnvironmentalProductDeclaration, (additionalObjectsResults[x] as EnvironmentalMetricResult).Quantity, field, objects);
            }
            results.AddRange(additionalObjectsResults);
        ...
        public static List<LifeCycleAssessmentElementResult> EvaluateLifeCycleAssessmentScope(this EnclosuresScope enclosuresScope, EnvironmentalProductDeclarationField field, List<LifeCycleAssessmentPhases> phases, bool exactMatch = false)
        {
            ...
            List<IElementM> walls = enclosuresScope.Walls.Select(x => x).ToList();
            ...
            for (int x = 0; x < enclWallsResults.Count; x++)
            {
                ...
                enclWallsResults[x] = new EnvironmentalMetricResult(enclWallsResults[x].ObjectId, enclWallsResults[x].ResultCase, enclWallsResults[x].TimeStep, ObjectScope.Enclosure, ObjectCategory.Wall, phases, enclWallsResults[x].EnvironmentalProductDeclaration, (enclWallsResults[x] as EnvironmentalMetricResult).Quantity, field, walls);
            }
            ...
            List<IElementM> curtain = enclosuresScope.CurtainWalls.Select(x => x).ToList();
            ...
            List<IElementM> windows = enclosuresScope.Windows.Select(x => x).ToList();
            ...
            for (int x = 0; x < windowsResults.Count; x++)
            {
                ...
                windowsResults[x] = new EnvironmentalMetricResult(windowsResults[x].ObjectId, windowsResults[x].ResultCase, windowsResults[x].TimeStep, ObjectScope.Enclosure, ObjectCategory.Window, phases, windowsResults[x].EnvironmentalProductDeclaration, (windowsResults[x] as EnvironmentalMetricResult).Quantity, field, windows);
            }
            ...
            List<IElementM> doors = enclosuresScope.Doors.Select(x => x).ToList();
            ...
            for (int x = 0; x < doorsResults.Count; x++)
            {
                ...
                doorsResults[x] = new EnvironmentalMetricResult(doorsResults[x].ObjectId, doorsResults[x].ResultCase, doorsResults[x].TimeStep, ObjectScope.Enclosure, ObjectCategory.Wall, phases, doorsResults[x].EnvironmentalProductDeclaration, (doorsResults[x] as EnvironmentalMetricResult).Quantity, field, doors);
            }
            ...
            List<IElementM> objects = enclosuresScope.AdditionalObjects.Select(x => x).ToList();
            ...
            for (int x = 0; x < additionalObjectsResults.Count; x++)
            {
                ...
                additionalObjectsResults[x] = new EnvironmentalMetricResult(additionalObjectsResults[x].ObjectId, additionalObjectsResults[x].ResultCase, additionalObjectsResults[x].TimeStep, ObjectScope.Enclosure, ObjectCategory.AdditionalObjects, phases, additionalObjectsResults[x].EnvironmentalProductDeclaration, (additionalObjectsResults[x] as EnvironmentalMetricResult).Quantity, field, objects);
            }
            results.AddRange(additionalObjectsResults);
        ...
        public static List<LifeCycleAssessmentElementResult> EvaluateLifeCycleAssessmentScope(this MEPScope mepScope, EnvironmentalProductDeclarationField field, List<LifeCycleAssessmentPhases> phases, bool exactMatch = false)
        {
            ...
            List<IElementM> airTerminals = mepScope.MechanicalScope.AirTerminals.Select(x => x).ToList();
            ...
            for (int x = 0; x < airTerminalResults.Count; x++)
            {
                ...
                airTerminalResults[x] = new EnvironmentalMetricResult(airTerminalResults[x].ObjectId, airTerminalResults[x].ResultCase, airTerminalResults[x].TimeStep, ObjectScope.Mechanical, ObjectCategory.AirTerminals, phases, airTerminalResults[x].EnvironmentalProductDeclaration, (airTerminalResults[x] as EnvironmentalMetricResult).Quantity, field, airTerminals);
            }
            results.AddRange(airTerminalResults);

            //MechanicalScope Dampers
            List<LifeCycleAssessmentElementResult> damperResults = mepScope.MechanicalScope.Dampers.Select(x => EvaluateEnvironmentalProductDeclaration(x, phases, field, exactMatch)).ToList();

            List<IElementM> damper = mepScope.MechanicalScope.Dampers.Select(x => x).ToList();

            for (int x = 0; x < damperResults.Count; x++)
            {
                if (damperResults[x] == null)
                {
                ...
                damperResults[x] = new EnvironmentalMetricResult(damperResults[x].ObjectId, damperResults[x].ResultCase, damperResults[x].TimeStep, ObjectScope.Mechanical, ObjectCategory.Dampers, phases, damperResults[x].EnvironmentalProductDeclaration, (damperResults[x] as EnvironmentalMetricResult).Quantity, field, damper);
            }
            ...
            List<IElementM> ducts = mepScope.MechanicalScope.Ducts.Select(x => x).ToList();
            ...
            for (int x = 0; x < ductResults.Count; x++)
            {
                ...
                ductResults[x] = new EnvironmentalMetricResult(ductResults[x].ObjectId, ductResults[x].ResultCase, ductResults[x].TimeStep, ObjectScope.Mechanical, ObjectCategory.Ducts, phases, ductResults[x].EnvironmentalProductDeclaration, (ductResults[x] as EnvironmentalMetricResult).Quantity, field, ducts);
            }
            ...
            List<IElementM> equipment = mepScope.MechanicalScope.Equipment.Select(x => x).ToList();
            ...
            for (int x = 0; x < mechanicalEquipmentResults.Count; x++)
            {
                ...
                mechanicalEquipmentResults[x] = new EnvironmentalMetricResult(mechanicalEquipmentResults[x].ObjectId, mechanicalEquipmentResults[x].ResultCase, mechanicalEquipmentResults[x].TimeStep, ObjectScope.Mechanical, ObjectCategory.Equipment, phases, mechanicalEquipmentResults[x].EnvironmentalProductDeclaration, (mechanicalEquipmentResults[x] as EnvironmentalMetricResult).Quantity, field, equipment);
            }
            ...
            List<IElementM> pipes = mepScope.MechanicalScope.Pipes.Select(x => x).ToList();
            ...
            for (int x = 0; x < pipingResults.Count; x++)
            {
                ...
                pipingResults[x] = new EnvironmentalMetricResult(pipingResults[x].ObjectId, pipingResults[x].ResultCase, pipingResults[x].TimeStep, ObjectScope.Mechanical, ObjectCategory.Pipes, phases, pipingResults[x].EnvironmentalProductDeclaration, (pipingResults[x] as EnvironmentalMetricResult).Quantity, field, pipes);
            }
            ...
            List<IElementM> refrigerants = mepScope.MechanicalScope.Refrigerants.Select(x => x).ToList();
            ...
            List<IElementM> tanks = mepScope.MechanicalScope.Tanks.Select(x => x).ToList();
            ...
            for (int x = 0; x < tankResults.Count; x++)
            {
                ...
                tankResults[x] = new EnvironmentalMetricResult(tankResults[x].ObjectId, tankResults[x].ResultCase, tankResults[x].TimeStep, ObjectScope.Mechanical, ObjectCategory.Tanks, phases, tankResults[x].EnvironmentalProductDeclaration, (tankResults[x] as EnvironmentalMetricResult).Quantity, field, tanks);
            }
            ...
            List<IElementM> valves = mepScope.MechanicalScope.Valves.Select(x => x).ToList();
            ...
            for (int x = 0; x < valveResults.Count; x++)
            {
                ...
                valveResults[x] = new EnvironmentalMetricResult(valveResults[x].ObjectId, valveResults[x].ResultCase, valveResults[x].TimeStep, ObjectScope.Mechanical, ObjectCategory.Valves, phases, valveResults[x].EnvironmentalProductDeclaration, (valveResults[x] as EnvironmentalMetricResult).Quantity, field, valves);
            }
            ...
            List<IElementM> objects = mepScope.MechanicalScope.AdditionalObjects.Select(x => x).ToList();
            ...
            List<IElementM> batteries = mepScope.ElectricalScope.Batteries.Select(x => x).ToList();
            ...
            List<IElementM> cableTrays = mepScope.ElectricalScope.CableTrays.Select(x => x).ToList();
            ...
            for (int x = 0; x < cableTrayResults.Count; x++)
            {
                ...
                cableTrayResults[x] = new EnvironmentalMetricResult(cableTrayResults[x].ObjectId, cableTrayResults[x].ResultCase, cableTrayResults[x].TimeStep, ObjectScope.Electrical, ObjectCategory.CableTray, phases, cableTrayResults[x].EnvironmentalProductDeclaration, (cableTrayResults[x] as EnvironmentalMetricResult).Quantity, field, cableTrays);
            }
            ...
            List<IElementM> conduit = mepScope.ElectricalScope.Conduit.Select(x => x).ToList();
            ...
            for (int x = 0; x < conduitResults.Count; x++)
            {
                ...
                conduitResults[x] = new EnvironmentalMetricResult(conduitResults[x].ObjectId, conduitResults[x].ResultCase, conduitResults[x].TimeStep, ObjectScope.Electrical, ObjectCategory.Conduit, phases, conduitResults[x].EnvironmentalProductDeclaration, (conduitResults[x] as EnvironmentalMetricResult).Quantity, field, conduit);
            }
            ...
            List<IElementM> electricalEquipment = mepScope.ElectricalScope.Equipment.Select(x => x).ToList();
            ...
            List<IElementM> fireAlarms = mepScope.ElectricalScope.FireAlarmDevices.Select(x => x).ToList();
            ...
            for (int x = 0; x < fireAlarmDeviceResults.Count; x++)
            {
                ...
                fireAlarmDeviceResults[x] = new EnvironmentalMetricResult(fireAlarmDeviceResults[x].ObjectId, fireAlarmDeviceResults[x].ResultCase, fireAlarmDeviceResults[x].TimeStep, ObjectScope.Electrical, ObjectCategory.FireAlarmDevices, phases, fireAlarmDeviceResults[x].EnvironmentalProductDeclaration, (fireAlarmDeviceResults[x] as EnvironmentalMetricResult).Quantity, field, fireAlarms);
            }
            ...
            List<IElementM> generators = mepScope.ElectricalScope.Generators.Select(x => x).ToList();
            ...
            for (int x = 0; x < generatorResults.Count; x++)
            {
                ...
                generatorResults[x] = new EnvironmentalMetricResult(generatorResults[x].ObjectId, generatorResults[x].ResultCase, generatorResults[x].TimeStep, ObjectScope.Electrical, ObjectCategory.Generator, phases, generatorResults[x].EnvironmentalProductDeclaration, (generatorResults[x] as EnvironmentalMetricResult).Quantity, field, generators);
            }
            ...
            List<IElementM> informationCommunicationDevices = mepScope.ElectricalScope.InformationCommunicationDevices.Select(x => x).ToList();
            ...
            List<IElementM> lightFixtures = mepScope.ElectricalScope.LightFixtures.Select(x => x).ToList();
            ...
            for (int x = 0; x < lightFixtureResults.Count; x++)
            {
                ...
                lightFixtureResults[x] = new EnvironmentalMetricResult(lightFixtureResults[x].ObjectId, lightFixtureResults[x].ResultCase, lightFixtureResults[x].TimeStep, ObjectScope.Electrical, ObjectCategory.LightFixtures, phases, lightFixtureResults[x].EnvironmentalProductDeclaration, (lightFixtureResults[x] as EnvironmentalMetricResult).Quantity, field, lightFixtures);
            }
            ...
            List<IElementM> lightControls = mepScope.ElectricalScope.LightingControls.Select(x => x).ToList();
            ...
            for (int x = 0; x < lightControlResults.Count; x++)
            {
                ...
                lightControlResults[x] = new EnvironmentalMetricResult(lightControlResults[x].ObjectId, lightControlResults[x].ResultCase, lightControlResults[x].TimeStep, ObjectScope.Electrical, ObjectCategory.LightingControls, phases, lightControlResults[x].EnvironmentalProductDeclaration, (lightControlResults[x] as EnvironmentalMetricResult).Quantity, field, lightControls);
            }
            ...
            List<IElementM> meters = mepScope.ElectricalScope.Meters.Select(x => x).ToList();
            ...
            for (int x = 0; x < meterResults.Count; x++)
            {
                ...
                meterResults[x] = new EnvironmentalMetricResult(meterResults[x].ObjectId, meterResults[x].ResultCase, meterResults[x].TimeStep, ObjectScope.Electrical, ObjectCategory.Meters, phases, meterResults[x].EnvironmentalProductDeclaration, (meterResults[x] as EnvironmentalMetricResult).Quantity, field, meters);
            }
            ...
            List<IElementM> securityDevices = mepScope.ElectricalScope.SecurityDevices.Select(x => x).ToList();
            ...
            List<IElementM> sockets = mepScope.ElectricalScope.Sockets.Select(x => x).ToList();
            ...
            for (int x = 0; x < socketResults.Count; x++)
            {
                ...
                socketResults[x] = new EnvironmentalMetricResult(socketResults[x].ObjectId, socketResults[x].ResultCase, socketResults[x].TimeStep, ObjectScope.Electrical, ObjectCategory.Sockets, phases, socketResults[x].EnvironmentalProductDeclaration, (socketResults[x] as EnvironmentalMetricResult).Quantity, field, sockets);
            }
            ...
            List<IElementM> solarPanels = mepScope.ElectricalScope.SolarPanels.Select(x => x).ToList();
            ...
            for (int x = 0; x < solarPanelResults.Count; x++)
            {
                ...
                solarPanelResults[x] = new EnvironmentalMetricResult(solarPanelResults[x].ObjectId, solarPanelResults[x].ResultCase, solarPanelResults[x].TimeStep, ObjectScope.Electrical, ObjectCategory.SolarPanels, phases, solarPanelResults[x].EnvironmentalProductDeclaration, (solarPanelResults[x] as EnvironmentalMetricResult).Quantity, field, solarPanels);
            }
            ...
            List<IElementM> wireSegments = mepScope.ElectricalScope.WireSegments.Select(x => x).ToList();
            ...
            for (int x = 0; x < wiringResults.Count; x++)
            {
                ...
                wiringResults[x] = new EnvironmentalMetricResult(wiringResults[x].ObjectId, wiringResults[x].ResultCase, wiringResults[x].TimeStep, ObjectScope.Electrical, ObjectCategory.Wiring, phases, wiringResults[x].EnvironmentalProductDeclaration, (wiringResults[x] as EnvironmentalMetricResult).Quantity, field, wireSegments);
            }
            ...
            List<IElementM> additionalElectrical = mepScope.ElectricalScope.AdditionalObjects.Select(x => x).ToList();
            ...
            List<IElementM> plumbingEquipment = mepScope.PlumbingScope.Equipment.Select(x => x).ToList();
            ...
            List<IElementM> plumbingPipes = mepScope.PlumbingScope.Pipes.Select(x => x).ToList();
            ...
            for (int x = 0; x < plumbingPipingResults.Count; x++)
            {
                ...
                plumbingPipingResults[x] = new EnvironmentalMetricResult(plumbingPipingResults[x].ObjectId, plumbingPipingResults[x].ResultCase, plumbingPipingResults[x].TimeStep, ObjectScope.Plumbing, ObjectCategory.Pipes, phases, plumbingPipingResults[x].EnvironmentalProductDeclaration, (plumbingPipingResults[x] as EnvironmentalMetricResult).Quantity, field, plumbingPipes);
            }
            ...
            List<IElementM> fixtures = mepScope.PlumbingScope.PlumbingFixtures.Select(x => x).ToList();
            ...
            for (int x = 0; x < plumbingFixtureResults.Count; x++)
            {
                ...
                plumbingFixtureResults[x] = new EnvironmentalMetricResult(plumbingFixtureResults[x].ObjectId, plumbingFixtureResults[x].ResultCase, plumbingFixtureResults[x].TimeStep, ObjectScope.Plumbing, ObjectCategory.PlumbingFixtures, phases, plumbingFixtureResults[x].EnvironmentalProductDeclaration, (plumbingFixtureResults[x] as EnvironmentalMetricResult).Quantity, field, fixtures);
            }
            ...
            List<IElementM> plumbingTanks = mepScope.PlumbingScope.Tanks.Select(x => x).ToList();
            ...
            for (int x = 0; x < plumbingTanksResults.Count; x++)
            {
                ...
                plumbingTanksResults[x] = new EnvironmentalMetricResult(plumbingTanksResults[x].ObjectId, plumbingTanksResults[x].ResultCase, plumbingTanksResults[x].TimeStep, ObjectScope.Plumbing, ObjectCategory.Tanks, phases, plumbingTanksResults[x].EnvironmentalProductDeclaration, (plumbingTanksResults[x] as EnvironmentalMetricResult).Quantity, field, plumbingTanks);
            }
            ...
            List<IElementM> plumbingValves = mepScope.PlumbingScope.Valves.Select(x => x).ToList();
            ...
            List<IElementM> additionalPlumbing = mepScope.PlumbingScope.AdditionalObjects.Select(x => x).ToList();
            ...
            for (int x = 0; x < additionalPlumbingObjectsResults.Count; x++)
            {
                ...
                additionalPlumbingObjectsResults[x] = new EnvironmentalMetricResult(additionalPlumbingObjectsResults[x].ObjectId, additionalPlumbingObjectsResults[x].ResultCase, additionalPlumbingObjectsResults[x].TimeStep, ObjectScope.Plumbing, ObjectCategory.AdditionalObjects, phases, additionalPlumbingObjectsResults[x].EnvironmentalProductDeclaration, (additionalPlumbingObjectsResults[x] as EnvironmentalMetricResult).Quantity, field, additionalPlumbing);
            }
            ...
            List<IElementM> fireEquipment = mepScope.FireProtectionScope.Equipment.Select(x => x).ToList();
            ...
            List<IElementM> firePipes = mepScope.FireProtectionScope.Pipes.Select(x => x).ToList();
            ...
            for (int x = 0; x < fireProtectionPipesResults.Count; x++)
            {
                ...
                fireProtectionPipesResults[x] = new EnvironmentalMetricResult(fireProtectionPipesResults[x].ObjectId, fireProtectionPipesResults[x].ResultCase, fireProtectionPipesResults[x].TimeStep, ObjectScope.FireProtection, ObjectCategory.Pipes, phases, fireProtectionPipesResults[x].EnvironmentalProductDeclaration, (fireProtectionPipesResults[x] as EnvironmentalMetricResult).Quantity, field, firePipes);
            }
            ...
            List<IElementM> fireSprinklers = mepScope.FireProtectionScope.Sprinklers.Select(x => x).ToList();
            ...
            List<IElementM> fireTanks = mepScope.FireProtectionScope.Tanks.Select(x => x).ToList();
            ...
            for (int x = 0; x < fireProtectionTankResults.Count; x++)
            {
                ...
                fireProtectionTankResults[x] = new EnvironmentalMetricResult(fireProtectionTankResults[x].ObjectId, fireProtectionTankResults[x].ResultCase, fireProtectionTankResults[x].TimeStep, ObjectScope.FireProtection, ObjectCategory.Tanks, phases, fireProtectionTankResults[x].EnvironmentalProductDeclaration, (fireProtectionTankResults[x] as EnvironmentalMetricResult).Quantity, field, fireTanks);
            }
            ...
            List<IElementM> fireObjects = mepScope.FireProtectionScope.AdditionalObjects.Select(x => x).ToList();
            ...
            for (int x = 0; x < additionalFireProtectionObjectsResults.Count; x++)
            {
                ...
                additionalFireProtectionObjectsResults[x] = new EnvironmentalMetricResult(additionalFireProtectionObjectsResults[x].ObjectId, additionalFireProtectionObjectsResults[x].ResultCase, additionalFireProtectionObjectsResults[x].TimeStep, ObjectScope.FireProtection, ObjectCategory.AdditionalObjects, phases, additionalFireProtectionObjectsResults[x].EnvironmentalProductDeclaration, (additionalFireProtectionObjectsResults[x] as EnvironmentalMetricResult).Quantity, field, fireObjects);
            }
        ...
        public static List<LifeCycleAssessmentElementResult> EvaluateLifeCycleAssessmentScope(this TenantImprovementScope tenantImprovementScope, EnvironmentalProductDeclarationField field, List<LifeCycleAssessmentPhases> phases, bool exactMatch = false)
        {
            ...
            List<IElementM> ceiling = tenantImprovementScope.Ceiling.Select(x => x).ToList();
            ...
            for (int x = 0; x < ceilingResults.Count; x++)
            {
                ...
                ceilingResults[x] = new EnvironmentalMetricResult(ceilingResults[x].ObjectId, ceilingResults[x].ResultCase, ceilingResults[x].TimeStep, ObjectScope.TenantImprovement, ObjectCategory.Ceiling, phases, ceilingResults[x].EnvironmentalProductDeclaration, (ceilingResults[x] as EnvironmentalMetricResult).Quantity, field, ceiling);
            }
            ...
            List<IElementM> finishes = tenantImprovementScope.Finishes.Select(x => x).ToList();
            ...
            for (int x = 0; x < finishResults.Count; x++)
            {
                ...
                finishResults[x] = new EnvironmentalMetricResult(finishResults[x].ObjectId, finishResults[x].ResultCase, finishResults[x].TimeStep, ObjectScope.TenantImprovement, ObjectCategory.Finishes, phases, finishResults[x].EnvironmentalProductDeclaration, (finishResults[x] as EnvironmentalMetricResult).Quantity, field, finishes);
            }
            ...
            List<IElementM> furniture = tenantImprovementScope.Furniture.Select(x => x).ToList();
            ...
            for (int x = 0; x < furnitureResults.Count; x++)
            {
                ...
                furnitureResults[x] = new EnvironmentalMetricResult(furnitureResults[x].ObjectId, furnitureResults[x].ResultCase, furnitureResults[x].TimeStep, ObjectScope.TenantImprovement, ObjectCategory.Furniture, phases, furnitureResults[x].EnvironmentalProductDeclaration, (furnitureResults[x] as EnvironmentalMetricResult).Quantity, field, furniture);
            }
            ...
            List<IElementM> intDoors = tenantImprovementScope.InteriorDoors.Select(x => x).ToList();
            ...
            for (int x = 0; x < intDoorsResults.Count; x++)
            {
                ...
                intDoorsResults[x] = new EnvironmentalMetricResult(intDoorsResults[x].ObjectId, intDoorsResults[x].ResultCase, intDoorsResults[x].TimeStep, ObjectScope.TenantImprovement, ObjectCategory.InteriorDoor, phases, intDoorsResults[x].EnvironmentalProductDeclaration, (intDoorsResults[x] as EnvironmentalMetricResult).Quantity, field, intDoors);
            }
            ...
            List<IElementM> intGlazing = tenantImprovementScope.InteriorGlazing.Select(x => x).ToList();
            ...
            List<IElementM> partWalls = tenantImprovementScope.PartitionWalls.Select(x => x).ToList();
            ...
            for (int x = 0; x < partWallsResults.Count; x++)
            {
                ...
                partWallsResults[x] = new EnvironmentalMetricResult(partWallsResults[x].ObjectId, partWallsResults[x].ResultCase, partWallsResults[x].TimeStep, ObjectScope.TenantImprovement, ObjectCategory.PartitionWall, phases, partWallsResults[x].EnvironmentalProductDeclaration, (partWallsResults[x] as EnvironmentalMetricResult).Quantity, field, partWalls);
            }
            ...
            List<IElementM> tenantObjects = tenantImprovementScope.AdditionalObjects.Select(x => x).ToList();
            ...
            for (int x = 0; x < additionalObjectsResults.Count; x++)
            {
                ...
                additionalObjectsResults[x] = new EnvironmentalMetricResult(additionalObjectsResults[x].ObjectId, additionalObjectsResults[x].ResultCase, additionalObjectsResults[x].TimeStep, ObjectScope.TenantImprovement, ObjectCategory.AdditionalObjects, phases, additionalObjectsResults[x].EnvironmentalProductDeclaration, (additionalObjectsResults[x] as EnvironmentalMetricResult).Quantity, field, tenantObjects);
            }
            results.AddRange(additionalObjectsResults);
